...
For HandLend, a feature slice is not considered complete until verification is recorded and the verdict is `PASS`. If UI, navigation, forms, or actor flows changed, verification must include Playwright browser execution.
...
If the change touches UI, navigation, forms, or actor flows:

```
Run Playwright browser verification
|-- Open a real browser
|-- Exercise the affected actor flow and screen IDs
|-- Compare the interaction against UX expectations from `UX Spec _ Product Design Document.md`
|-- Capture concise evidence for the verify report
`-- Flag: CRITICAL if browser behavior blocks the specified flow
```
...
For HandLend in `openspec` mode, verification is incomplete until `progress.md` reflects the new status and links the verify artifact.
...
- For UI, navigation, form, and actor-flow changes, ALWAYS include Playwright browser evidence in the verification report
- A new HandLend feature slice MUST NOT begin unless the previous slice has a recorded verification verdict of `PASS`
