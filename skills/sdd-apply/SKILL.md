...
For HandLend, implement only one feature slice at a time. Do not start work on the next slice until the current slice has updated progress, linked verification evidence, and a `PASS` verdict in its prior `verify-report`.
...
5. Read `SDD Handlend.md` to confirm where this slice belongs in the implementation sequence
6. If the task touches UI, read `UX Spec _ Product Design Document.md` and use `Ant Design` as the only UI library
...
|-- Write the code
|-- Run the relevant tests before marking the task complete
`-- Note any issues or deviations
```

### Step 4: Mark Tasks Complete

Update `tasks.md` - change `- [ ]` to `- [x]` for completed tasks only after the required automated checks for that task have been executed successfully:
...
For HandLend in `openspec` mode, also update the repo-level `progress.md` entry for the current slice with actor, screen, status, linked artifact, last verification date, and brief evidence.
...
- ALWAYS use `Ant Design` for UI implementation work unless the user explicitly changes the rule
- ALWAYS validate UI decisions against `UX Spec _ Product Design Document.md`
...
- For UI, navigation, form, and actor-flow changes, ALWAYS prepare or run Playwright verification before considering the slice ready for verify
- NEVER move to a new feature slice while the current slice lacks a recorded `verify-report` with verdict `PASS`
